module quadpack
    ...
    Use quadpack_constants, Only : wp
    
    implicit none

contains

! reproduit le comportement de la fonction dummy f
! mais pour des arguments scalaires. 
! retourne egalement un argument scalaire.
function f_scal(f, x_scal)
    interface
        function f(x)
                Use quadpack_constants, Only : wp
                intrinsic size
    ...
    real(kind=wp), intent(in) :: x_scal
    real(kind=wp) :: f_scal

    real(kind=wp), dimension(1) :: f_array
...
subroutine aaaa
! http://orion.math.iastate.edu/burkardt/f_src/quadpack/quadpack.html
!
!******************************************************************************
!
! 1. introduction
...
!  Parameters:
!
!    Output, real(kind=wp) PI, the value of pi.
!
  implicit none
!
  real(kind=wp) pi
...
!  Parameters:
!
!    Input, external real(kind=wp) F, the name of the function routine, of the form
!      function f ( x )
!      real(kind=wp) f
!      real(kind=wp) x
!    which evaluates the integrand function.
!
!    Input, real(kind=wp) A, B, the limits of integration.
!
!    Input, real(kind=wp) EPSABS, EPSREL, the absolute and relative accuracy requested.
!
...
!
!    Output, real(kind=wp) RESULT, the estimated value of the integral.
!
!    Output, real(kind=wp) ABSERR, an estimate of || I - RESULT ||.
!
...
!
  real(kind=wp)  :: a
  real(kind=wp)  :: abserr
  real(kind=wp)  :: alist(limit)
  real(kind=wp)  :: b
  real(kind=wp)  :: blist(limit)
  real(kind=wp)  :: elist(limit)
  real(kind=wp)  :: epsabs
  real(kind=wp)  :: epsrel
...
!    6, the input is invalid, because EPSABS < 0 and EPSREL < 0.
!
!    Workspace, real(kind=wp) ALIST(LIMIT), BLIST(LIMIT), contains in entries 1 
!    through LAST the left and right ends of the partition subintervals.
!
!    Workspace, real(kind=wp) RLIST(LIMIT), contains in entries 1 through LAST
!    the integral approximations on the subintervals.
!
!    Workspace, real(kind=wp) ELIST(LIMIT), contains in entries 1 through LAST
...
!    which evaluates the integrand function.
!
!    Input, real(kind=wp) BOUND, the value of the finite endpoint of the integration
...
!   real(kind=wp), external :: f
  ...
  real(kind=wp) reseps
  real(kind=wp) result
  real(kind=wp) res3la(3)
  real(kind=wp) rlist(limit)
  real(kind=wp) rlist2(52)
  real(kind=wp) small
!
!  Test on validity of parameters.
!
...
!
  ...
  call qk15i ( f, boun, inf, 0.0e+00_wp, 1.0e+00_wp, result, abserr, defabs, resabs )
!
!  Test on accuracy.
!
  last = 1
...
! !
! !    Input/output, real(kind=wp) POINTS(NPTS2), contains the user provided interior
...
! !    Input, real(kind=wp) A, B, the limits of integration.
! !
! !    Input, real(kind=wp) C, a parameter in the weight function, which must
...
!   interface
!      function f(x)
!             Use Use quadpack_constants, Only : wp_constants, Only : wp
!             intrinsic size
!             real(kind=wp)             :: x(:)
!             real(kind=wp)             :: f(size(x))
!      end function f
...
! !    Input, real(kind=wp) A, B, the limits of integration.
! !
! !    Input, real(kind=wp) C, a parameter in the weight function, which cannot be
! !    equal to A or B.
! !
! !    Input, real(kind=wp) EPSABS, EPSREL, the absolute and relative accuracy requested.
! !
...
! !    which evaluates the integrand function.
! !
! !    Input, real(kind=wp) A, the lower limit of integration.
! !
! !    Input, real(kind=wp) OMEGA, the parameter in the weight function.
! !
! !    Input, integer INTEGR, indicates which weight functions is used
! !    = 1, w(x) = cos(omega*x)
! !    = 2, w(x) = sin(omega*x)
! !
! !    Input, real(kind=wp) EPSABS, the absolute accuracy requested.
! !
! !    Output, real(kind=wp) RESULT, the estimated value of the integral.
! !
! !    Output, real(kind=wp) ABSERR, an estimate of || I - RESULT ||.
...
!   real(kind=wp) alist(limit)
!   real(kind=wp) blist(limit)
!   real(kind=wp) chebmo(maxp1,25)
!   real(kind=wp) elist(limit)
!   real(kind=wp) epsabs
!   real(kind=wp) erlst(limlst)
! !   real(kind=wp), external :: f
!   integer ier
!   integer integr
!   integer iord(limit)
...
! !
! !            alist, blist, rlist, elist - real(kind=wp)
...
!   integer numrl2
!   real(kind=wp) omega
!   real(kind=wp), parameter :: p = 0.9E+00
!   real(kind=wp), parameter :: pi = 3.1415926535897932E+00
!   real(kind=wp) p1
!   real(kind=wp) psum(52)
!   real(kind=wp) reseps
!   real(kind=wp) result
!   real(kind=wp) res3la(3)
!   real(kind=wp) rlist(limit)
...
! 
!     if ( integr == 1 ) then
!       call qagi ( f, 0.0e+00_wp, 1, epsabs, 0.0e+00_wp, result, abserr, neval, ier )
!     else
!       result = 0.0E+00
!       abserr = 0.0E+00
!       neval = 0
...
!     epsa = eps*fact
! 
!     call qfour ( f, c1, c2, omega, integr, epsa, 0.0e+00_wp, limit, lst, maxp1, &
...
! !    Input, real(kind=wp) A, B, the limits of integration.
! !
! !    Input, real(kind=wp) ALFA, BETA, parameters used in the weight function.
...
! !    Input, real(kind=wp) A, B, the limits of integration.
! !
! !    Input, real(kind=wp) C, the parameter in the weight function.
! !
! !    Output, real(kind=wp) RESULT, the estimated value of the integral.
...
!   real(kind=wp) cc
!   real(kind=wp) centr
!   real(kind=wp) cheb12(13)
!   real(kind=wp) cheb24(25)
! !   real(kind=wp), external :: f
!   real(kind=wp) fval(25)
!   real(kind=wp) hlgth
!   integer i
!   integer isym
!   integer k 
...
!   real(kind=wp) res24
!   real(kind=wp) u
!   real(kind=wp), parameter, dimension ( 11 ) :: x = (/ &
...
!     u = hlgth * x(i-1)
!     isym = 26 - i
!     fval(i) = sum(f(u+centr)
!     fval(isym) = sum(f(centr-u)
!   end do
! !
...
! !    Output, integer NEVAL, the number of times the integral was evaluated.
! !
! !    Output, real(kind=wp) RESABS, approximation to the integral J.
! !
! !    Output, real(kind=wp) RESASC, approximation to the integral of abs(F-I/(B-A)).
! !
! !         on entry and return
! !           momcom - integer
...
!             intrinsic size
!             real(kind=wp)             :: x(:)
!             real(kind=wp)             :: omega, p2, p3, p4
!             integer          :: integr
!             real(kind=wp)             :: qwgto(size(x))
!      end function qwgto
! 
!      function f(x)
!             Use quadpack_constants, Only : wp
...
! !    Input, real(kind=wp) A, B, the limits of integration.
! !
! !    Input, real(kind=wp) BL, BR, the lower and upper limits of integration.
! !    A <= BL < BR <= B.
! !
! !    Input, real(kind=wp) ALFA, BETA, parameters in the weight function.
! !
! !    Input, real(kind=wp) RI(25), RJ(25), RG(25), RH(25), modified Chebyshev moments 
! !    for the application of the generalized Clenshaw-Curtis method,
! !    computed in QMOMO.
! !
! !    Output, real(kind=wp) RESULT, the estimated value of the integral, computed by 
...
! !    Output, real(kind=wp) ABSERR, an estimate of || I - RESULT ||.
! !
! !    Output, real(kind=wp) RESASC, approximation to the integral of abs(F*W-I/(B-A)).
! !
...
!             intrinsic size
!             real(kind=wp)             :: x(:)
!             real(kind=wp)             :: a, b, alfa, beta
!             integer          :: integr
!             real(kind=wp)             :: qwgts(size(x))
!      end function qwgts
! 
!      function f(x)
!             Use quadpack_constants, Only : wp
...
! !  Parameters:
! !
! !    Input, real(kind=wp) X(11), contains the values of COS(K*PI/24), for K = 1 to 11.
! !
! !    Input/output, real(kind=wp) FVAL(25), the function values at the points
...
!
!    Input/output, real(kind=wp) EPSTAB(52), the two lower diagonals of the triangular
!    epsilon table.  The elements are numbered starting at the right-hand 
!    corner of the triangle.
!
!    Output, real(kind=wp) RESULT, the estimated value of the integral.
!
!    Output, real(kind=wp) ABSERR, estimate of the absolute error computed from
!    RESULT and the 3 previous results.
!
!    ?, real(kind=wp) RES3LA(3), the last 3 results.
!
...
! !    Input, real(kind=wp) A, B, the limits of integration.
! !
! !    Input, real(kind=wp) OMEGA, the multiplier of X in the weight function.
! !
...
!    Output, real(kind=wp) ABSERR, an estimate of | I - RESULT |.
!
!    Output, real(kind=wp) RESABS, approximation to the integral of the absolute
!    value of F.
!
!    Output, real(kind=wp) RESASC, approximation to the integral | F-I/(B-A) | 
!    over [A,B].
!
!  Local Parameters:
!
...
!  and estimate the absolute error.
  ...
  do j = 1, 3
    jtw = j*2
    absc = hlgth*xgk(jtw)
    fval1 = f_scal(f, centr-absc)
    fval2 = f_scal(f, centr+absc)
    fv1(jtw) = fval1
    fv2(jtw) = fval2
    fsum = fval1+fval2
    resg = resg+wg(j)*fsum
...
!    which evaluates the integrand function.
!
!    Input, real(kind=wp) BOUN, the finite bound of the original integration range,
!    or zero if INF is 2.
!
!              inf    - integer
...
!                       over (0,+infinity).
!
!    Input, real(kind=wp) A, B, the limits of integration, over a subrange of [0,1].
!
!    Output, real(kind=wp) RESULT, the estimated value of the integral.
...
!    value of F.
!
!    Output, real(kind=wp) RESASC, approximation to the integral of the
...
!
  ...
  fval1 = f_scal(f,tabsc1)
  if ( inf == 2 ) fval1 = fval1+f_scal(f,-tabsc1)
...
!
  ...
  do j = 1, 7
    ...
    fval1 = f_scal(f,tabsc1)
    fval2 = f_scal(f,tabsc2)

    if ( inf == 2 ) then
      fval1 = fval1+f_scal(f,-tabsc1)
      fval2 = fval2+f_scal(f,-tabsc2)
    end if
...
! !                       calling program.
! !
! !    ?, real(kind=wp) P1, P2, P3, P4, parameters in the weight function
! !
! !              kp     - integer
...
!   real(kind=wp) reskh
!   real(kind=wp) result
!   real(kind=wp), external :: w
!   real(kind=wp), dimension ( 4 ) :: wg = (/ &
!     1.294849661688697e-01,     2.797053914892767e-01, &
...
! !  Parameters:
! !
! !    Input, real(kind=wp) ALFA, a parameter in the weight function w(x), ALFA > -1.
! !
! !    Input, real(kind=wp) BETA, a parameter in the weight function w(x), BETA > -1.
! !
! !           ri     - real(kind=wp)
...
!   real(kind=wp) resasc
!   real(kind=wp) reskh
!   real(kind=wp) savfun(21)
!   real(kind=wp) w10(5)
!   real(kind=wp) w21a(5)
!   real(kind=wp) w21b(6)
!   real(kind=wp) w43a(10)
!   real(kind=wp) w43b(12)
!   real(kind=wp) w87a(21)
!   real(kind=wp) w87b(23)
!   real(kind=wp) x1(5)
!   real(kind=wp) x2(5)
!   real(kind=wp) x3(11)
!   real(kind=wp) x4(22)
...
!    largest error.
!
!    Output, real(kind=wp) ERMAX, the NRMAX-th largest error = ELIST(MAXERR).
!
!    Input, real(kind=wp) ELIST(LIMIT), contains the error estimates.
!
...
! !  Parameters:
! !
! !    Input, real(kind=wp) X, the point at which the weight function is evaluated.
! !
! !    Input, real(kind=wp) C, the location of the singularity.
! !
! !    Input, real(kind=wp) P2, P3, P4, parameters that are not used.
! !
! !    Input, integer KP, a parameter that is not used.
! !
! !    Output, real(kind=wp) QWGTC, the value of the weight function at X.
! !
!   implicit none
! !
!   real(kind=wp) c
...
! !    Input, real(kind=wp) X, the point at which the weight function is evaluated.
! !
! !    Input, real(kind=wp) OMEGA, the factor multiplying X.
! !
! !    Input, real(kind=wp) P2, P3, P4, parameters that are not used.
! !
...
! !    2, W(X) = sin ( OMEGA * X )
! !
! !    Output, real(kind=wp) QWGTO, the value of the weight function at X.
! !
!   implicit none
! !
!   integer integr
...
! !    Input, real(kind=wp) X, the point at which the weight function is evaluated.
! !
! !    Input, real(kind=wp) A, B, the endpoints of the integration interval.
! !
! !    Input, real(kind=wp) ALFA, BETA, exponents that occur in the weight function.
! !
! !    Input, integer INTEGR, specifies which weight function is used:
! !    1. W(X) = (X-A)**ALFA * (B-X)**BETA
! !    2, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A)
...
! !    4, W(X) = (X-A)**ALFA * (B-X)**BETA * log (X-A) * log(B-X)
! !
! !    Output, real(kind=wp) QWGTS, the value of the weight function at X.
! !
!   implicit none
! !
!   real(kind=wp) a
...
! !*******************************************************************************
! !
! !! R_SWAP swaps two real(kind=wp) values.
! !
! !
! !  Modified:
! !
...
! !  Parameters:
! !
! !    Input/output, real(kind=wp) X, Y.  On output, the values of X and
