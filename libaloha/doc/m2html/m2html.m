...
%  $Revision$Date: 2003/31/08 17:25:20 $
...
%-------------------------------------------------------------------------------
%- Get list of M-files, MEX-files, C-Files, F-Files and H-Files
%-------------------------------------------------------------------------------
if ~options.load
    excludedFiles = {'AUTHORS.m','INSTALL.m','README.m','VERSIONS.m'};
	mfiles = getmfiles(options.mFiles,{},options.recursive,excludedFiles);
	...
	if options.verbose,
        ...
        for i=1:length(mfiles)
            [dummy, dummmy,ext] = fileparts(mfiles{i});
            switch ext
                case {'.c' '.cpp' '.c++' '.cxx' '.C'}
                    lcfile = lcfile + 1;
                case {'.for' '.f' '.FOR' '.F'}
        ...
        fprintf('Found %d C-files.\n',lcfile);
        fprintf('Found %d F-files.\n',lffile);
        fprintf('Found %d H-files.\n',lhfile);
	end
...
if ~options.load
	...
	mdir = unique(mdirs);
	if options.verbose,
		fprintf('Found %d unique directories.\n',length(mdir));
	end
...
if ~options.load
    ...
    isc        = zeros(length(mfiles),length(cexts));
    isf        = zeros(length(mfiles),length(fexts));
    ish        = zeros(length(mfiles),length(hexts));  
...
if options.save
	...
	save(fullfile(options.htmlDir,matfilesave), ...
		...
		'hrefs', 'synopsis', 'h1line', 'subroutine', 'todo', 'ismex','isc','isf','ish');
end
...
cKeywords =      {'include','unsigned','register','const','static','define','void','typedef','char','int','double','struct','else', ...
				  'end', 'for', 'function',  'if', ...
				  'return','while','complex'};
fKeywords =      {'elseif','else','common','real','integer','logical', 'then','call','external'...
				  'for','function','if','implicit','enddo','do','write','read','format','subroutine','parameter', ...
				  'return','while','continue','endif','save','close','end','character','double','precision','intrinsic','data'};

strtok_delim = sprintf(' \t\n\r(){}[]<>+-*~!|\\@&/,:;="''%');%- Delimiters used in strtok: some of them may be useless (% " .), removed '.'
...
tpl_mfile_include  = '<span class="include">%s</span>';
...
for i=1:length(mdir)
	for j=1:length(mdirs)
		if strcmp(mdirs{j},mdir{i})
            ...
            % Ajout JH 16/10/2003
            % Sans cette re-generation de l'extenstion 'ext', il y'a une confusion
            % avec les fichiers temporaires .m~ generes par l'editeur de fichier kate...
            % Et dans ce cas, les fichiers .m ne seraient pas parses.
            [dummy, dummy, ext] = fileparts(mfiles{j}); 

            if strcmp(ext,'.m')%for M-file only                 
                ...
                if options.source & ~strcmpi(names{j},'contents')
                    ...
                    while 1
                        ...
                        if ~ischar(tline), break, end
                        myline = '';
                        splitc = splitcodem(entity(tline));
            ...
            %
            if strcmp(ext,'.f') | strcmp(ext,'.F') | strcmp(ext,'.for') | strcmp(ext,'.FOR'), %for F-files

                
               tpl = set(tpl,'var','DESCRIPTION',horztab('Fortran source',options.tabs));
               tpl = set(tpl,'var','H1LINE_CALLED', ' ');
               
               tpl = set(tpl,'var','crossrefcalleds','');
               tpl = set(tpl,'var','crossrefcalls','');
               tpl = set(tpl,'var','subf','');
               tpl = set(tpl,'var','onesubf','');
               
               % 
               %- Display source code with highlight fortran keywords
               %
                if options.source & ~strcmpi(names{j},'contents')
                    fseek(fid2,0,-1);
                    it = 1;
                    ...
                    while 1
                        ...
                        if ~ischar(tline), break, end
                        myline = '';
                        splitc = splitcodef(entity(tline));                       
                        for k=1:length(splitc)
                            if isempty(splitc{k})
                            elseif splitc{k}(1) == ''''
                                myline = [myline ...
                                ...
                                while 1
                                    ...
                                    myline = [myline q];
                                    if isempty(t), break, end;
                                    %- Highlight fortran keywords
                                  
                                    if options.syntaxHighlighting & ~strcmp(' ',tline(1)),%Comment line in fortran 
                                        myline = [sprintf(tpl_mfile_comment,tline)];break
                                    else
                                        if options.syntaxHighlighting & any(strcmp(fKeywords,lower(t)))
                                            if strcmp('end',t)
                                                rr = fliplr(deblank(fliplr(r)));
                                                icomma = strmatch(',',rr);
                                                isemicolon = strmatch(';',rr);
                        ...
                        fsource = [fsource sprintf(tpl_mfile_line,it,myline)];
                        it = it + 1;
                    end
                    tpl = set(tpl,'var','SOURCECODE',...
                              horztab(fsource,options.tabs));
                    tpl = parse(tpl,'thesource','source');
                else
                    tpl = set(tpl,'var','thesource','');
                end
            ...
            %
            if strcmp(ext,'.c') | strcmp(ext,'.C') | strcmp(ext,'.c++') | strcmp(ext,'.cpp') | strcmp(ext,'.cxx') | strcmp(ext,'.h') | strcmp(ext,'.H'), %for C-files or H-files

               if strcmp(ext,'.c') | strcmp(ext,'.C') | strcmp(ext,'.c++') | strcmp(ext,'.cpp') | strcmp(ext,'.cxx'), tpl = set(tpl,'var','DESCRIPTION',horztab('C/C++ source',options.tabs));end
               if strcmp(ext,'.h') | strcmp(ext,'.H'), tpl = set(tpl,'var','DESCRIPTION',horztab('Header source',options.tabs));end
               tpl = set(tpl,'var','H1LINE_CALLED', ' ');
               ...
               %
                if options.source & ~strcmpi(names{j},'contents')
                    ...
                    while 1
                        tline = fgetl(fid2); 
                        iline = iline+1;
                       % if iline == 348,keyboard,end
                        if ~ischar(tline), break, end
                        myline = '';
                        splitc = splitcodec(tline);
                        for k=1:length(splitc)
                            if isempty(splitc{k})
                            elseif splitc{k}(1) == '"'
                                myline = [myline sprintf(tpl_mfile_string,entity(splitc{k}))];                                                                    
                            elseif splitc{k}(1) == ''''  & ~flag_comment
                                myline = [myline sprintf(tpl_mfile_string,entity(splitc{k}))];                                                                    
                             else
                                %- Look for keywords
                                r = splitc{k};
                                ...
                                while 1                                       
                                    if size(rr,2) >= 1,
                                        if options.syntaxHighlighting & strcmp('#',rr(1)) & ~flag_comment, 
                                            myline = [myline sprintf(tpl_mfile_include,entity(rr))];%Include part 
                                            break;
                                        end
                                    end
                                    if size(rr,2) >= 2,
                                        if options.syntaxHighlighting & strcmp('//',rr(1:2)),
                                            myline = [myline sprintf(tpl_mfile_comment,entity(rr))];%Include part 
                                            break;
                                        end                                      
                                        if options.syntaxHighlighting & strcmp('/*',rr(1:2)),
                                            myline = [myline sprintf(tpl_mfile_comment,entity(rr))];%Include part 
                                    ...
                                    if options.syntaxHighlighting & flag_comment,
                                        myline = [myline sprintf(tpl_mfile_comment,entity(rr))];%Include part 
                                        break;
                                    end     
                                    [t,r,q] = mstrtok(r,strtok_delim);
                                    ...
                                    if options.syntaxHighlighting & any(strcmp(cKeywords,lower(t)))
                                        if strcmp('end',t)
                                            rr = fliplr(deblank(fliplr(r)));
                                            icomma = strmatch(',',rr);
                                            isemicolon = strmatch(';',rr);
...
%===============================================================================
function mfiles = getmfiles(mdirs,mfiles,recursive,excludedFiles)
	%- Extract M-files, MEX-files, C-files, F-files and H-files from a list of directories and/or M-files
    if nargin < 4,
        excludedFiles = [];
    end
    %
	for i=1:length(mdirs)
		...
		elseif exist(mdirs{i}) == 7 % Directory
			w = what_aloha_yp(mdirs{i});
			w = w(1); %- Sometimes an array is returned...
			for j=1:length(w.m)
                excludedFile_flag = 0;
                for k = 1:length(excludedFiles),
                    if strcmp(w.m{j},excludedFiles{k}),
                        excludedFile_flag = 1;
                    end
                end
                if ~excludedFile_flag,
                    mfiles{end+1} = fullfile(mdirs{i},w.m{j});%.m files
                end
            end
            for j=1:length(w.mex)
				mfiles{end+1} = fullfile(mdirs{i},w.mex{j});%.mex files
			end
            for j=1:length(w.c)
				mfiles{end+1} = fullfile(mdirs{i},w.c{j});%.c files
			end
            for j=1:length(w.h)
				mfiles{end+1} = fullfile(mdirs{i},w.h{j});%.h files
            end
            for j=1:length(w.f)
				mfiles{end+1} = fullfile(mdirs{i},w.f{j});%.f files
            end
			...
			if recursive
				...
				for j=1:length(d)
					mfiles = getmfiles(cellstr(fullfile(mdirs{i},d{j})),...
									   mfiles,recursive,excluded_files);
				end
			end
		else
