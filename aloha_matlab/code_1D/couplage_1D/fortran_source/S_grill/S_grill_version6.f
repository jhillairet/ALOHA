...
C Declaration de variable
      ...
      Common/idem/bcte
      Common/onde/k0
      Common/plasma/X0,D0,d_couche,X1,D1,pertes
      Common/integration/max_nz

ccccccccccccccccccccccccccccccccccccccccccc
cccccc modif altair.partenaires ccccccccccc
...
C Nb. max. de guide du Grill = 60
      ...
      Real *8 X0,D0,d_couche,X1,D1,pertes,max_nz
...
c&&&C Nb. max. de guide du Grill = 60
...
c&&&      Real *8 X0,D0,d_couche,X1,D1,pertes,max_nz
...
cccccc modif altair.partenaires ccccccccccc
      ...
      Real *8 k0,X0,D0,d_couche,X1,D1,pertes
...
c&&&      Integer *4 i,j,m,n,v,ifail,kount
c&&&      Real *8 k0,X0,D0,d_couche,X1,D1,pertes
c&&&      Real *8 nz,z(15),b(15),pi
c&&&      Real *8 coeff_mode_m,coeff_mode_n
c&&&      Real *8 PC_limit
c&&&      Complex *16 neta,alpha,Ai,Aid
c&&&      Complex *16 Ai_0,Aid_0,Bi_0,Bid_0
c&&&      Complex *16 Ai_d,Aid_d,Bi_d,Bid_d
c&&&      Complex *16 y_L,ya,yb,ys,ys_sur_factor,num,denom
c&&&      
c&&&      External S17DGF,S17DHF
...
cccccccccccccccccccccccccccccccccccccccccccc
      ...
      neta=((1.,0.)*nz**2-alpha)**(1./3.)*(D1/X1)**(2./3.)*
     &     (X1-1.)*CDEXP(-(0.,1.)*pi/3.)
     ...
     &     (X0-1.)*CDEXP(-(0.,1.)*pi/3.)
      ...
      ifail=1
      Call S17DGF('f',neta,'u',Ai_0,v,ifail)
      ifail=1
      Call S17DGF('d',neta,'u',Aid_0,v,ifail)

      If (CDABS(Ai_0).LT.PC_limit) Then
          f_nz=(0.,0.)
	  goto 10	  
      Else      
          ya=-alpha*(Aid_0/Ai_0)*(X0/D0)**(1./3.)/
     &           ((1.,0.)*nz**2-alpha)**(2./3.)*
     &           CDEXP((0.,1.)*pi/6.)
      ...
      ifail=1
      Call S17DHF('f',neta,'u',Bi_0,ifail)
      ifail=1
      Call S17DHF('d',neta,'u',Bid_0,ifail)

      If (CDABS(Bi_0).LT.PC_limit) Then
          f_nz=(0.,0.)
	  goto 10	  
      Else      
          yb=-alpha*(Bid_0/Bi_0)*(X0/D0)**(1./3.)/
     &           ((1.,0.)*nz**2-alpha)**(2./3.)*
     &           CDEXP((0.,1.)*pi/6.)
      ...
      neta=((1.,0.)*nz**2-alpha)**(1./3.)*(X0/D0)**(1./3.)*
     &     (k0*d_couche+(D0/X0)*(X0-1.))*CDEXP(-(0.,1.)*pi/3.)
         
      ifail=1
      Call S17DGF('f',neta,'u',Ai_d,v,ifail)
      ifail=1
      Call S17DGF('d',neta,'u',Aid_d,v,ifail)
      ifail=1
      Call S17DHF('f',neta,'u',Bi_d,ifail)
      ifail=1
      Call S17DHF('d',neta,'u',Bid_d,ifail)

      If ((CDABS(Bid_0).LT.PC_limit).OR.
     &         (CDABS(Aid_0).LT.PC_limit)) Then
          f_nz=(0.,0.)
	  goto 10	  
      Else 
          num=y_L*(Ai_d/Ai_0*yb-Bi_d/Bi_0*ya)+
     &  	  ya*yb*(Bid_d/Bid_0-Aid_d/Aid_0)
          denom=y_L*(Ai_d/Ai_0-Bi_d/Bi_0)-
     &  	  Aid_d/Aid_0*ya+Bid_d/Bid_0*yb
...
c&&&      Real *8 z(15),b(15)
c&&&                
c&&&      Integer *4 LW,LIW,IW(5000)           
c&&&      Integer *4 ifail
c&&&      Real *8 W(20000)
c&&&      Real *8 epsrel,err_r,max_nz
c&&&      Real *8 min,max,resultr,resulti
c&&&      Real *8 rf_nz,if_nz
c&&&      External rf_nz,if_nz
...
cccccc modif altair.partenaires ccccccccccc
      ...
      Real *8 X0,D0,d_couche,X1,D1,pertes
...
c&&&      Real *8 a,b(15),z(15),k0,Z0,Y0
c&&&      Real *8 X0,D0,d_couche,X1,D1,pertes
