...
C Nb. max. de guide du Grill = 60
      ...
      Real *8 D0,X0,pertes,d_vide,max_nz
...
C ***
      ...
      Real *8 k0,X0,D0,pertes,d_vide
      Real *8 nz,z(60),b(60),pi
      Real *8 coeff_mode_m,coeff_mode_n
      Real *8 PC_limit
      Complex *16 neta,Ai,Aid,Bi,Bid,alpha,gamma,th
      Complex *16 ys_x_0,ys,y,ys_sur_factor
     ...
     &           CDEXP((0.,1.)*pi/6.)
      ...
      gamma=k0*(nz**2-alpha)**(1./2.) 
      y=(0.,1.)*k0/gamma
      
      th=(CDEXP(2*gamma*d_vide)-1)/(CDEXP(2*gamma*d_vide)+1)      
      ys=y*(ys_x_0+y*th)/(ys_x_0*th+y)
