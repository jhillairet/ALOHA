...
C Declaration de variable
      ...
      Common/idem/bcte
      Common/onde/k0
      Common/plasma/X0,D0,pertes,S_dielectric,P_dielectric
...
C Nb. max. de guide du Grill = 60
      ...
      Real *8 S_dielectric,P_dielectric
...
C ***
      ...
      Real *8 k0,X0,D0,pertes,S,P
      Real *8 nz,z(60),b(60),pi,sign_z
      Real *8 coeff_mode_m,coeff_mode_n
      Real *8 S_dielectric,P_dielectric
      Complex *16 alpha,gamma
      Complex *16 ys,ys_sur_factor,nxS
...
c      alpha=(1.,0.)-(0.,1.)*pertes
c      ys=(0.,1.)*alpha/sqrt((1.,0.)*nz**2-alpha)

      
      
c      If (pertes.EQ.0) Then
c      
c          If (DABS(nz).EQ.1) Then
c 
c              f_nz=(0.,0.)
c	      goto 10
c	      
...
c         alpha=(1.,0.)-(0.,1.)*pertes
c     
c         gamma=sqrt(((1.,0.)*X0 - alpha)*((1.,0.)*nz**2-alpha)/alpha)
c       
c         If (DIMAG(gamma).LT.0) Then
c      
c         ys=-alpha*gamma/((1.,0.)*nz**2-alpha)
c	 
c         Else
c      
c         ys=alpha*gamma/((1.,0.)*nz**2-alpha)
...
c      Endif
      ...
      If (DABS(S_dielectric-nz**2).LT.1.E-4) Then
      ...
      If (((P_dielectric/S_dielectric)*
     &             (S_dielectric-nz**2)).LT.0.) Then
     
         nxS=(0.,1.)*DSQRT(DABS((P_dielectric/S_dielectric)*
     &	                                  (S_dielectric-nz**2)))
      
      Else
      
         sign_z=DABS(S_dielectric/(S_dielectric-nz**2))*
     &	                        (S_dielectric-nz**2)/S_dielectric
         nxS=-sign_z*DSQRT((P_dielectric/S_dielectric)*
     &	                                    (S_dielectric-nz**2))
           
      Endif      

      ys=-nxS*S_dielectric/
     &              (S_dielectric-(nz**2)*((1.,0.)+1.E-6*(0.,1.)))    
