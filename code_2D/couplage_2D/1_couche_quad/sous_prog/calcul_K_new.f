...
c programme
      ...
      distance = DSQRT((y2-y1)**2 + (z2-z1)**2)
      lambda = 2.*pi/k0

c      If (distance.GT.(4.*lambda)) Then
c         K = (0.,0.)
c      Else
c         quad_key=3
c         error=1.E-3
c         Call integral_discr(K) 
c      Endif



      If ((z1.LE.1.E-3).AND.(y1.LE.1.E-3).AND.(y2.LE.1.E-3).AND.
     &    (m1.EQ.1).AND.(n1.EQ.0)) Then
...
c intégration
     ...
     &        neval,ier,limit,lenw,last,iwork,work)
      ...
      Integer *4 quad_key
      Real *8 nz_quad,error,omega_y,omega_z,nz
      Real *8 real_F_nz,imag_F_nz

      External real_F_nz,imag_F_nz

      real_G_nz = real_F_nz(nz)*DCOS(omega_z*nz) +  
     &                         imag_F_nz(nz)*DSIN(omega_z*nz)
      ...
      imag_G_nz = imag_F_nz(nz)*DCOS(omega_z*nz) -  
     &                         real_F_nz(nz)*DSIN(omega_z*nz)
      ...
      Integer *4 npts2,key,neval,ier,limit,leniw,lenw,maxp,last
      integer *4 iwork(800),quad_key
      Real *8 a_nz,b_nz,epsabs,epsrel,epsabsp,epsrelp,real_G_nz,imag_G_nz
      Real *8 Kr,points(2),abserr,work(2000),S,D,dD,P,dP,K_tamp 
      Real *8 borne_max,demi_intervalle,error,nz_quad,omega_y,omega_z

      External real_G_nz,imag_G_nz
      ...
      a_nz=-borne_max
      b_nz=-DSQRT(DABS(S))-demi_intervalle
      Call dqawo(real_G_nz,a_nz,b_nz,omega_z,1,epsabs,epsrel,K_tamp,
     &     abserr,neval,ier,leniw,maxp,lenw,last,iwork,work)
      Kr=K_tamp
      Call dqawo(imag_G_nz,a_nz,b_nz,omega_z,2,epsabs,epsrel,K_tamp,
     &     abserr,neval,ier,leniw,maxp,lenw,last,iwork,work)
      Kr=Kr-K_tamp
     ...
     &     abserr,neval,ier,leniw,maxp,lenw,last,iwork,work)
      ...
      a_nz=-borne_max
      b_nz=-DSQRT(DABS(S))-demi_intervalle
      Call dqawo(real_G_nz,a_nz,b_nz,omega_z,2,epsabs,epsrel,K_tamp,
     &     abserr,neval,ier,leniw,maxp,lenw,last,iwork,work)
      Ki=K_tamp
      Call dqawo(imag_G_nz,a_nz,b_nz,omega_z,1,epsabs,epsrel,K_tamp,
     &     abserr,neval,ier,leniw,maxp,lenw,last,iwork,work)
      Ki=Ki+K_tamp
     ...
     &     abserr,neval,ier,leniw,maxp,lenw,last,iwork,work)
      ...
      If ((DABS(omega_y).LE.25.).AND.(DABS(omega_z).LE.25.)) Then
      ...
      Else
         ...
         error=1.E-4
         Call integral_real_g(Kr)
         Call integral_imag_g(Ki)
         K = (1.,0.)*Kr + (0.,1.)*Ki
...
c ********************************************
c fonction parabolique (parfois à des facteurs près ; voir abramovitz)
c le rapport dérivée/fonction est correct
...
c ********************************************
c fonction Whittaker (parfois à des facteurs près ; voir abramovitz)
c le rapport dérivée/fonction est correct
...
c valable a priori pour |a|<5 et |x|<5 => étendu via des tests numériques
...
c ********************************************
c fonction airy (parfois à des facteurs près ; voir abramovitz)
c le rapport dérivée/fonction est correct
